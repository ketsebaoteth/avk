 ...
 * Processes instanced immediate-mode shapes and records commands directly into
 * active command streams.
 */
class Renderer {
public:
  ...
  /**
   * @brief Records the drawing commands into a Vulkan command list using
   * Dynamic Rendering.
   * @param cmd Current frame command list.
   * @param targetView The output image view (e.g. current swapchain render
   * target).
   * @param targetFormat Color format of the render target (required for
   * pipeline verification).
   * @param extent Pixel boundary of the output canvas.
   */
  void render(VkCommandBuffer cmd, VkImageView targetView,
