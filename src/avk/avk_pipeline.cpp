...
PipelineCache::PipelineCache(VulkanContext* context) : m_context(context) {
    if (!m_context || !m_context->isValid()) {
        std::cerr << "avk: Cannot initialize PipelineCache with an invalid VulkanContext." << std::endl;
        return;
    }

    // 1. Define Push Constants (screen width, screen height)
    VkPushConstantRange pushConstantRange{};
    pushConstantRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    pushConstantRange.offset = 0;
    pushConstantRange.size = sizeof(float) * 2;

    // 2. Define Set Layout Layout
    ...
    vkCreateDescriptorSetLayout(m_context->getDevice(), &descriptorLayoutInfo, nullptr, &dummyLayout);

    // 3. Create Pipeline Layout
    VkPipelineLayoutCreateInfo pipelineLayoutInfo{};
    pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    pipelineLayoutInfo.setLayoutCount = 1;
    pipelineLayoutInfo.pSetLayouts = &dummyLayout;
    ...
    vkDestroyDescriptorSetLayout(m_context->getDevice(), dummyLayout, nullptr);
}
...
VkPipeline PipelineCache::getOrCreatePipeline(VkFormat colorFormat) {
    ...
    if (vertModule == VK_NULL_HANDLE || fragModule == VK_NULL_HANDLE) {
        std::cerr << "avk: Failed to compile graphics pipeline due to missing shader assets." << std::endl;
    ...
    std::vector<VkVertexInputAttributeDescription> attributes = {
        ...
        {3, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(InstanceData, borderRadius)},
        ...
        {6, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(InstanceData, strokeColor)},
        ...
        {9, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(InstanceData, strokeFillColorA)},
        {10, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(InstanceData, strokeFillColorB)},
...
VkShaderModule PipelineCache::loadShaderModule(const std::string& fileName) {
    ...
    if (!file.is_open()) {
        std::cerr << "avk: Failed to open compiled shader binary file: " << fileName << std::endl;
        return VK_NULL_HANDLE;
    }
    ...
    if (vkCreateShaderModule(m_context->getDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS) {
        std::cerr << "avk: Failed to generate Shader Module for " << fileName << std::endl;
    }
    return shaderModule;
}
