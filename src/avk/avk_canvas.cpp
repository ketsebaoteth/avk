...
namespace avk {
    ...
    WindowCanvas::WindowCanvas(VulkanContext* context, VkSurfaceKHR surface, uint32_t width, uint32_t height)
        : m_context(context)
        , m_width(width)
        , m_height(height) {

        m_swapchain = std::make_unique<VulkanSwapchain>(m_context, surface, width, height);
    ...
    bool WindowCanvas::isActive() const {
        return m_swapchain->isActive();
    }
    ...
    bool WindowCanvas::beginFrame() {
        ...
        transitionImageLayout(
            cmd,
            m_swapchain->getImages()[m_acquiredImageIndex],
    ...
    void WindowCanvas::endFrame(Renderer& renderer) {
        ...
        renderer.render(
            cmd,
            m_swapchain->getImageViews()[m_acquiredImageIndex],
        ...
        vkQueueSubmit(m_context->getGraphicsQueue(), 1, &submitInfo, frame.getInFlightFence());
    ...
    void WindowCanvas::transitionImageLayout(VkCommandBuffer cmd, VkImage image, VkImageLayout oldLayout, VkImageLayout newLayout) {
