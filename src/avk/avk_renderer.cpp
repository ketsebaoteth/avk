...
Renderer::Renderer(VulkanContext* context) : m_context(context) {
    if (!m_context || !m_context->isValid()) {
        std::cerr << "avk: Cannot initialize Renderer with an invalid VulkanContext." << std::endl;
        return;
    }
    ...
    // We map it permanently on creation using VMA flags to allow sequential-write host-access.
...
void Renderer::submit(const InstanceData& instance) {
    if (m_drawQueue.size() >= m_maxInstances) {
        std::cerr << "avk: Draw queue limit exceeded! Max capacity: " << m_maxInstances << std::endl;
        return;
    }
    m_drawQueue.push_back(instance);
}

void Renderer::render(VkCommandBuffer cmd, VkImageView targetView, VkFormat targetFormat, VkExtent2D extent) {
    ...
    std::memcpy(
        m_instanceBuffer.getMappedData(), 
        m_drawQueue.data(), 
    ...
    // We use LOAD to draw the UI transparently on top of existing background renderings
    ...
    float screenSize[2] = { static_cast<float>(extent.width), static_cast<float>(extent.height) };
    vkCmdPushConstants(
        cmd, 
        m_pipelineCache->getPipelineLayout(), 
...
void Renderer::buildQuadBuffer() {
    ...
    std::memcpy(m_quadVertexBuffer.getMappedData(), quadVertices.data(), bufferSize);
}
