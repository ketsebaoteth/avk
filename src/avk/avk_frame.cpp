...
FrameContext::FrameContext(VulkanContext* context) : m_context(context) {
    if (!m_context || !m_context->isValid()) {
        std::cerr << "avk: Cannot initialize FrameContext with an invalid VulkanContext." << std::endl;
        return;
    }
    ...
    if (vkCreateCommandPool(device, &poolInfo, nullptr, &m_commandPool) != VK_SUCCESS) {
        std::cerr << "avk: Failed to create command pool for FrameContext." << std::endl;
        return;
    }
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, &m_commandBuffer) != VK_SUCCESS) {
        std::cerr << "avk: Failed to allocate command buffer for FrameContext." << std::endl;
        release();
        return;
    }
    ...
    if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &m_imageAvailableSemaphore) != VK_SUCCESS ||
        ...
        std::cerr << "avk: Failed to create synchronization semaphores." << std::endl;
        release();
        return;
    }
...
FrameContext& FrameContext::operator=(FrameContext&& other) noexcept {
    if (this != &other) {
        ...
        other.m_renderFinishedSemaphore = VK_NULL_HANDLE;
...
bool FrameContext::reset() {
    if (vkResetCommandPool(m_context->getDevice(), m_commandPool, 0) != VK_SUCCESS) {
