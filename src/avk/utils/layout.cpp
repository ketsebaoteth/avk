...
Clay_ElementId getNextId(const char *name, uint32_t seed) {
  Clay_String str{false, static_cast<int32_t>(std::strlen(name)), name};
...
// ----------------------------------------------------------------------------
PositioningContextGuard::PositioningContextGuard(
    uint32_t elementId, atomic::Position positionType) {
...
PositioningContextGuard::PositioningContextGuard(
    PositioningContextGuard &&other) noexcept
    : active(other.active) {
  other.active = false;
}
  ...
  if (this != &other) {
    if (active) {
      ...
      if (uiState && !uiState->positioningContextStack.empty()) {
        uiState->positioningContextStack.pop_back();
      }
    }
    active = other.active;
    other.active = false;
...
// StyleCascadeGuard
// ----------------------------------------------------------------------------
StyleCascadeGuard::StyleCascadeGuard(const atomic::Style &style) {
  auto *uiState = atomic::getUiState();
  if (uiState) {
...
StyleCascadeGuard::~StyleCascadeGuard() {
  if (active) {
    auto *uiState = atomic::getUiState();
...
StyleCascadeGuard::StyleCascadeGuard(StyleCascadeGuard &&other) noexcept
    : active(other.active) {
  other.active = false;
}
...
// createFramePayload
// ----------------------------------------------------------------------------
atomic::RenderPayload *
createFramePayload(const atomic::Style &style, std::optional<float> scale,
                   std::optional<float> rotation, float textOffset,
                   uint32_t textureIndex, const glm::vec4 &tintColor) {
